...
MeshRenderer::MeshEntry::MeshEntry(std::vector<Vertex> vertices,
...
MeshRenderer::MeshEntry::~MeshEntry()
{
}

MeshRenderer::MeshRenderer(std::string filename, float unitConversion) :
SceneNode(),
...
void MeshRenderer::initMesh(unsigned int index, const aiMesh* mesh, float unitConversion)
{
...
void MeshRenderer::initMaterials(const aiScene* scene, std::string filename)
{
...
void MeshRenderer::setCustomBoundingBox(BoundingBox bb) {
    _boundingBox = bb;
}

// Centers the mesh within it's parent. Warning! This resets the position.
// Recommended to wrap the mesh in a container
void MeshRenderer::centerMesh() {
    vec3 halfSize = _boundingBox.getSize() * 0.5f;
    float xToCenter = halfSize.x - _boundingBox.maxX();
    float yToCenter = halfSize.y - _boundingBox.maxY();
    float zToCenter = halfSize.z - _boundingBox.maxZ();
    
    setPosition(vec3(xToCenter, yToCenter, zToCenter));
}
